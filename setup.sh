...
docker run -p 3900:3306 --name mysql -e MYSQL_ROOT_PASSWORD=icingaproject -d mysql:latest

git clone https://github.com/Sam-Urai/LinuxAdmin && cd test-deploy && docker build -t test-deploy/local:apache .
